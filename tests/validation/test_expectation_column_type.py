...
class TestColumnTypeExpectation:
    ...
    def test_init_basic_type(self, spark, create_test_df):
        """Test initialization with basic type"""
        df = create_test_df(["1", "2", "3"])
        expectation = ColumnTypeExpectation(spark, df, "test_column", "int")

        assert expectation.column_name == "test_column"
        assert expectation.target_type == "int"
        assert expectation.date_format is None

    def test_init_date_type_without_format(self, spark, create_test_df):
        """Test initialization fails for date type without format"""
        df = create_test_df(["2024-01-01"])

        with pytest.raises(ValueError) as exc_info:
            ColumnTypeExpectation(spark, df, "test_column", "date")
        assert "date_format is required" in str(exc_info.value)

    def test_init_timestamp_type_without_format(self, spark, create_test_df):
        """Test initialization fails for timestamp type without format"""
        df = create_test_df(["2024-01-01 12:00:00"])

        with pytest.raises(ValueError) as exc_info:
            ColumnTypeExpectation(spark, df, "test_column", "timestamp")
        assert "date_format is required" in str(exc_info.value)

    def test_validate_integer_conversion(self, spark, create_test_df):
    ...
    def test_validate_double_conversion(self, spark, create_test_df):
        """Test validation of double conversion"""
        df = create_test_df(["1.5", "2.0", "invalid", "3.7"])
        expectation = ColumnTypeExpectation(spark, df, "test_column", "double")
    ...
    def test_validate_timestamp_conversion(self, spark, create_test_df):
        """Test validation of timestamp conversion"""
        df = create_test_df(["2024-01-01 12:00:00", "invalid", "2024-12-31 23:59:59"])
        expectation = ColumnTypeExpectation(
            spark, df, "test_column", "timestamp", date_format="yyyy-MM-dd HH:mm:ss"
        )
    ...
    def test_validate_boolean_conversion(self, spark, create_test_df):
        """Test validation of boolean conversion"""
        df = create_test_df(["true", "false", "invalid", "TRUE", "FALSE"])
        expectation = ColumnTypeExpectation(spark, df, "test_column", "boolean")
    ...
    def test_validate_null_values(self, spark, create_test_df):
    ...
    def test_validate_empty_string(self, spark, create_test_df):
    ...
    def test_validate_invalid_column(self, spark, create_test_df):
        """Test validation with non-existent column"""
        df = create_test_df(["1", "2", "3"])
        expectation = ColumnTypeExpectation(spark, df, "non_existent_column", "int")

        with pytest.raises(ValueError) as exc_info:
            expectation.validate(df)
        assert "Column non_existent_column not found" in str(exc_info.value)

    def test_validate_preserves_original_values(self, spark, create_test_df):
    ...
    def test_validate_custom_date_format(self, spark, create_test_df):
        """Test validation with custom date format"""
        df = create_test_df(["01/31/2024", "invalid", "12/31/2024"])
        expectation = ColumnTypeExpectation(
            spark, df, "test_column", "date", date_format="MM/dd/yyyy"
        )
    ...
    def test_validation_result_properties(self, spark, create_test_df):
    ...
    def test_validate_multiple_types(
        self, spark, create_test_df, target_type, values, expected_flags
    ):
        """Test validation with different types using parametrize"""
        df = create_test_df(values)
        expectation = ColumnTypeExpectation(spark, df, "test_column", target_type)
